...
module.exports.streamURL = async (url, type) => {
  const axios = require('axios');
  const fs = require('fs');
  return axios.get(url, { responseType: 'arraybuffer' }).then(res => {
  const path = __dirname + `/../modules/commands/video/${Date.now()}.${type}`;
...
module.exports.trans = async function (lang, text){
...
module.exports.postContent = async function (url, from = {}){
  from.method = "POST"
  from.url = url
  return await axios(from).then(res => res.data)
}
